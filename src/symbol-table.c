...
LABEL *g_hash_table[HTABLE_SIZE];

static uint32_t stab_hash(char *s)
{
  uint32_t char_sum = 0;
  uint32_t result;
  while (*s)
...
{
  LABEL *result;
  for (result = g_hash_table[stab_hash(label)];
...
{
  LABEL *result = stab_add_label(task_name, /*addr_set*/ true, task_addr, /*is_task*/ true);
  return result;
}

LABEL *stab_add_forward_ref_task_label(char *task_name)
{
  LABEL *result = stab_add_label(task_name, /*addr_set*/ false, /*addr*/ 0, /*is_task*/ true);
  return result;
}

LABEL *stab_add_jump_label(char *label_name, uint32_t jump_addr)
{
  LABEL *result = stab_add_label(label_name, /*addr_set*/ true, jump_addr, /*is_task*/ false);
  return result;
}
